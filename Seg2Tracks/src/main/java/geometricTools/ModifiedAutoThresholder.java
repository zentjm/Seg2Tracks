...
public class ModifiedAutoThresholder {

//TODO: Make sure it is properly cited
/** Autothresholding methods (limited to 256 bin histograms) from the Auto_Threshold plugin 
    ...
    public int getThreshold(Method method, int[] histogram) {
        ...
        switch (method) {
            ...
            case OtsuBounded: threshold = OtsuBounded(histogram); break;
    ...
    int GHT(int [] data) {
        ...
        int k,j, kStar;  // k = the current threshold; j = backwards current threshold, kStar = optimal threshold
        double N1, N2,  N;    // N1 = # points with intensity <=k; N1 = # of remaining points with intensity, N = total number of points
        double BCV, BCVmax; // The current Between Class Variance and maximum BCV
        double num, denom;  // temporary bookeeping
        double Sk1, Sk2;  // The total intensity for all histogram points <=k and >k
        double S, L=256; // The total intensity of the image
        double M1, M2; // Mean of intensity of points before and after current threshold;
        ...
        for (k=1; k<L-1; k++) { // No need to check endpoints k = 0 or k = L-1
            ...
            if (denom != 0 ){
                ...
                num = ( (double)N1 / N ) * S - Sk1;  // Maximum value of num =  255*N = approx 8E7
                BCV = (num * num) / denom;
            }
            else
                BCV = 0;

            if (BCV >= BCVmax){ // If a better threshold
            	
            	Sk2 = 0;
            	N2 = 0;
            	for (j = k; j<L-1; j++) { //Begin checking for boundary condition
            		Sk2 += (double)j * data[j];
            		N2 += data[j];
            	}
            	M1 = Sk1/N1;
            	...
            	//System.out.println("Sk1: " + Sk1 + "  Sk2: " + Sk2);
            	//System.out.println("N1: " + N1 + "  N2: " + N2);
            	//System.out.println("M1: " + M1 + "  M2: " + M2);
            	
            	//if (histogram[trs] < mean1 && histogram[trs] < mean2) {
            	if (data[k] < M1 && data[k] < M2) {
            		BCVmax = BCV;
                    kStar = k;
            	}
            }   
    ...
    int MinErrorI(int [] data ) {
        ...
        while (threshold!=Tprev){
            ...
            sqterm = (w1*w1)-w0*w2;
            if (sqterm < 0) {
                System.out.println("MinError(I): not converging.");
                return threshold;
            }
