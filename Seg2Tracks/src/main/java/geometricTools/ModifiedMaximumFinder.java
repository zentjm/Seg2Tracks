...
public class ModifiedMaximumFinder implements ExtendedPlugInFilter, DialogListener {
    //filter params
    ...
    /** In recursive mode: the parent cell's polygon ROI, used to bound the scan and treat its boundary as an image edge */
    private PolygonRoi recursionParentRoi = null;
     ...
     */
    /**
     * Sets the parent cell polygon ROI for recursive segmentation mode.
     * When set, the raster scan is restricted to this ROI's bounding box and pixels
     * adjacent to any zero-valued neighbor (the masked boundary) are treated as edge
     * pixels and excluded from the candidate maxima list, the same way image-border
     * pixels are excluded when excludeOnEdges is true.
     * Pass null to clear (non-recursive runs).
     */
    public void setRecursionParentRoi(PolygonRoi roi) {
        this.recursionParentRoi = roi;
    }

    public Polygon getMaxima(ImageProcessor ip, double tolerance, boolean excludeOnEdges) {
        if (recursionParentRoi != null)
            ip.setRoi(recursionParentRoi);  // restrict raster scan to parent cell bounding box
    ...
    public Polygon getMaxima(ImageProcessor ip, double tolerance, boolean strict, boolean excludeOnEdges) {
		findMaxima(ip, tolerance, strict, ImageProcessor.NO_THRESHOLD,
			ModifiedMaximumFinder.POINT_SELECTION, excludeOnEdges, false);
    ...
    long[] getSortedMaxPoints(ImageProcessor ip, ByteProcessor typeP, boolean excludeEdgesNow,
        ...
        for (int y=roi.y; y<roi.y+roi.height; y++) {         // find local maxima now
            ...
            for (int x=roi.x, i=x+y*width; x<roi.x+roi.width; x++, i++) {      // for better performance with rois, restrict search to roi
                ...
                // In recursive mode: exclude pixels adjacent to the masked exterior from the
                // candidate list so the zeroed boundary ring does not generate spurious maxima.
                if (excludeEdgesNow && recursionParentRoi != null) {
                    boolean atBoundary = false;
                    for (int d = 0; d < 8; d++) {
                        int nx = x + DIR_X_OFFSET[d];
                        int ny = y + DIR_Y_OFFSET[d];
                        if (nx < 0 || nx >= width || ny < 0 || ny >= height || ip.get(nx, ny) == 0) {
                            atBoundary = true;
                            break;
                        }
                    }
