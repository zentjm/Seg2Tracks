...
/** This class is adapted from ImageJ's wand (tracing) tool.
 *	 See https://imagej.nih.gov/ij/developer/source/ij/gui/Wand.java.html for details
 */

public class ModifiedWand  {
    ...
    private boolean externalDependence;
	
    private Roi externalOutline;
	
	public ModifiedWand(ImageProcessor ip) {
	   this.ip = ip;
       ...
       width = ip.getWidth();
       height = ip.getHeight();
       externalDependence = false;
	}
	
	//Allows exclusion using the external segmentation outline.
	public ModifiedWand(ImageProcessor ip, Roi externalOutline) {
		this(ip);
		this.externalOutline = externalOutline;
    ...
    //determines if it is inside the external outline.
    private boolean insideExternal(int x, int y) {
    	if (externalOutline.contains(x, y)) return true;
		return false;
    }
    ...
    private boolean inside(int x, int y) {
    	...
    	//checks external
    	if (externalDependence) {
    		if (externalOutline == null || !insideExternal(x,y)) 
    			return false;
    	}
    	return true;
    }
