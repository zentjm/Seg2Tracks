...
public class MinimalBoundary extends ExternalSegmentation {

	public MinimalBoundary() {
		this.name = "Minimal Boundry";
	...
	Point[] outerPoints(Point[] innerPoints) {
		...
		//Add corner points
		/*
		pts.add(new PolarPoint((new Point(0, 0)), centerPoint));
		pts.add(new PolarPoint((new Point(0, processor.getHeight() - 1)), centerPoint));
		pts.add(new PolarPoint((new Point(processor.getWidth() - 1, 0)), centerPoint));
		pts.add(new PolarPoint((new Point(processor.getWidth() - 1, processor.getHeight() - 1)), centerPoint));
		*/
	...
	*/
		...
		while (!complete) {
			...
			for (int i = 0; i < outerPoints.size(); i ++) {	
				...
				int j = i + 1;
				if (i == outerPoints.size() - 1) j = 0; //circular condition
				
				//get points of bounding rectangle
				double r1 = outerPoints.get(i).getR();
				double r3 = outerPoints.get(j).getR();
				double theta1 = outerPoints.get(i).getTheta();
				double theta3 = outerPoints.get(j).getTheta();
				
				//collect all points within angle
				ArrayList<PolarPoint> tempList = new ArrayList<PolarPoint>();
				for (int k = 0; k < ptArray.length; k++) {
					if (outerPoints.contains(ptArray[k])) continue;
					double theta = ptArray[k].getTheta();
					if (theta1 < theta && theta < theta3) {
						tempList.add(ptArray[k]);
					}
				}
				if (tempList.size() == 0) continue;
				

				//finds smallest point
				double tempR2 = Double.MAX_VALUE;
				PolarPoint tempPt2 = null;
				for (int k = 0; k < tempList.size(); k++) {
					if (tempList.get(k).getR() < tempR) {
						tempR = tempList.get(k).getR();
						tempPt = tempList.get(k);
					}
				}
				
				//Adds closest point
				double r2 = tempPt.getR();
				double theta2 = tempPt.getTheta();
				
				double theta1x = Math.abs(theta2 - theta1);
				double r1x = Math.sqrt(Math.pow(r1,2) + Math.pow(r2,2) - (2 * r1 * r2 * Math.cos(theta1x)));
				double theta1x_M = theta1x; //- (Math.PI/2);
				//double angle1 = Math.asin((r1x * Math.sin(theta1x_M) / r2));
				double angle1 = Math.asin((r2 * Math.sin(theta1x_M) / r1x));
				
				double theta3x = Math.abs(theta3 - theta2);
				double r3x = Math.sqrt(Math.pow(r2, 2) + Math.pow(r3,2) - (2 * r2 * r3 * Math.cos(theta3x)));
				double theta3x_M = theta3x; // - (Math.PI/2);
				double angle3 = Math.asin((r2 * Math.sin(theta3x_M) / r3x));
				
				System.out.println("theta1: " + theta1 + "   r1: " + r1);
				System.out.println("theta2: " + theta2 + "   r2: " + r2);
				System.out.println("theta1x: " + theta1x + "   r1x: " + r1x);
				System.out.println("theta1x_M: " + theta1x_M);
				System.out.println("angle1: " + angle1);
				
				if (angle1 < (Math.PI/2) && angle3 < (Math.PI/2)) {
					outerPoints.add(j, tempPt);
					complete = false;
					System.out.println("Found match, outerPoints size: " + outerPoints.size());
				}				
			}
			...
			/*
			double thetaThird = ptArray[k].getTheta();
			if (theta1 < theta && theta < theta3) {
				tempList.add(ptArray[k]);
			}
			*/
		...
		Arrays.sort(ptArray, new Comparator<PolarPoint>() {
			public int compare(PolarPoint a, PolarPoint b) {
				return (a.getTheta() < b.getTheta() ? -1 : a.getTheta() > b.getTheta() ? 1 : 0);
			}
		});
		...
		/*
		//add second point
		PolarPoint maxPt = outerPoints.get(0);
		double maxTheta = 0;
		double tempTheta;
		for (int i = 0; i < ptArray.length; i++) {
			tempTheta = ptArray[i].getTheta() - outerPoints.get(0).getTheta();
			if (tempTheta < Math.PI && tempTheta > maxTheta) {
				maxTheta = tempTheta;
				maxPt = ptArray[i];
			}
		}
		outerPoints.add(maxPt);
		
		//add third point 
		maxPt = outerPoints.get(1);
		maxTheta = 0;
		for (int i = 0; i < ptArray.length; i++) {
			tempTheta = ptArray[i].getTheta() - outerPoints.get(1).getTheta();
			if (tempTheta < Math.PI && tempTheta > maxTheta) {
				maxTheta = tempTheta;
				maxPt = ptArray[i];
			}
		...
		*/
		...
		/*IF NEED FULL LIST
		//Convert to array for sortingrrrrr 
		PolarPoint[] tempPtArray = new PolarPoint[tempList.size()];
		for (int k = 0; k < tempList.size(); k++) {
			tempPtArray[k] = tempList.get(k);
		}
		
		//Sort array by r //TODO: Maybe just need to get the smallest r instead of a list
		Arrays.sort(tempPtArray, new Comparator<PolarPoint>() {
			public int compare(PolarPoint a, PolarPoint b) {
				return (a.getR() < b.getR() ? -1 : a.getR() > b.getR() ? 1 : 0);
			}
		});
		*/
		...
		while (!complete) {
			for (int i = 0; i < ptArray.length; i ++) {
				if (outerPoints.contains(ptArray[i])) continue; 
				complete = true;
				for (int j = 0; j < outerPoints.size(); j ++) {	
					
					int k = j + 1;
					if (j == outerPoints.size() - 1) k = 0; //circular condition
				
					//get points of bounding rectangle
					double r1 = outerPoints.get(j).getR();
					double r2 = outerPoints.get(k).getR();
					double theta1 = outerPoints.get(j).getTheta();
					double theta2 = outerPoints.get(k).getTheta();
					
					//collect all points within this angle
				
					double r = ptArray[i].getR();
					double theta = ptArray[i].getTheta();
					
					//else:
					if (theta1 < theta && theta < theta2) {
							
						
						System.out.println("Line crossover for ptArray " + i);
						
						double theta1x = theta - theta1;
						if (theta1x > Math.PI) theta1x = theta1x - Math.PI;
						double r1x = Math.sqrt(Math.pow(r1, 2) + Math.pow(r,2) - (2 * r1 * r * Math.cos(theta1x)));
						theta1x = theta1x - (Math.PI/2);
						double angle1 = Math.asin((r1x * Math.sin(theta1x) / r));
						
						
						
						double theta2x = theta2 - theta;
						if (theta2x > Math.PI) theta2x = theta2x - Math.PI;
						double r2x = Math.sqrt(Math.pow(r, 2) + Math.pow(r2,2) - (2 * r * r2 * Math.cos(theta2x)));
						theta2x = theta2x - (Math.PI/2);
						double angle2 = Math.asin((r2x * Math.sin(theta2x) / r));
